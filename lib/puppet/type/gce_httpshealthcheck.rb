...
Puppet::Type.newtype(:gce_httpshealthcheck) do
  desc 'Google Compute Engine HTTPS health check for load balanced instances'
  ...
  newparam(:description) do
    desc 'An optional, textual description for the HTTPS health check.'
  end
  ...
  newparam(:timeout) do
    desc "If Google Compute Engine doesn't receive an HTTPS 200 response from the instance by the time specified by the value of this flag, the health check request is considered a failure."
  end
  ...
  newparam(:host) do
    desc 'The value of the host header used in this HTTPS health check request.'
  end
